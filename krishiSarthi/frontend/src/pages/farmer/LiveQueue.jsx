...
<section className="rounded-2xl border border-[#d5ddd3] bg-white p-5"> 
  ...
  <div className="mt-4 grid gap-3 sm:grid-cols-2"> 
    ...
    <DynamicCounter 
      ...
      token={getCounterToken(queue, "Registration")} 
    /> 
 
    <DynamicCounter 
      name="Counter 2" 
      functionName="Quality Check" 
      token={getCounterToken( 
        queue, 
        "Quality Check", 
        a13TurnStarted, 
        a13Stage 
...
function getCounterToken( 
  queue, 
  stage, 
  a13TurnStarted = false, 
  a13Stage = "" 
) { 
  // A-13 is already in its own procurement workflow 
  if (a13TurnStarted) { 
    if (a13Stage === stage) { 
      return "A-13" 
    } 
